...
По умолчанию сейчас — G1 GC.

#### Generational GC 

Идея: большинство объектов живут недолго. Поэтому выгодно собирать «много» памяти часто в области, где живут молодые объекты, а старые — реже. Это уменьшает число объектов, которые нужно просматривать при сборке. Большинство JVM использует поколенческую модель: Young (eden + survivor) и Old/Tenured. Minor (young) GC — частые, быстрые; Major/Full — реже и дороже.

#### Copying collector (классический молодёжный GC)

Алгоритм (semi-space copying):

- Делим область на «from» и «to».

- При сборке живые объекты копируются из from → to (evacuation), при этом выполняется компактирование по ходу (новая область плотная).

- Освобождается вся from целиком (быстро и без фрагментации).
  - Плюсы: быстрые выделения (pointer-bump), отсутствие фрагментации в собираемой области, простая реализация.
  - Минусы: требует «пустого» пространства (половина heap для semi-space в простой реализации), неэффективен для долгоживущих объектов.

#### Regionalized copying — G1 и региональные сборщики

Идея: вместо крупных semi-spaces разбить кучу на много маленьких регионов (обычно одинакового размера, например 1–32MB), и собирать отдельные регионы по приоритету (те, где больше мусора). Это уменьшает накладные расходы и делает сбор более предсказуемым. G1 — классический пример.

Как работает G1 (схематично):

- Heap = набор регионов (young regions + old regions + humongous regions).

- Young-GC = evacuation выбранных young-регионов: живые объекты копируются в свободные регионы-назначения.

- G1 ведёт remembered sets (RSet) — структуры, которые хранят ссылки между регионами (чтобы не сканировать весь heap). RSet + card table уменьшают работу при эвакуации.

- Есть concurrent marking (несколько стадий: initial mark (STW), concurrent mark, remark (STW), cleanup), чтобы определить, какие регионы содержат много мусора → выбрать их для эвакуации (garbage-first). G1 стремится к целевым паузам (pause-goal).

- Humongous objects (больше чем регион) занимают целое/несколько регионов и обрабатываются отдельно.

#### Mark-and-Sweep vs Mark-and-Compact (и mark-copy)

Mark-and-Sweep: 1) mark — пометить достижимые объекты от корней; 2) sweep — пробежать и освобождать все немаркированные объекты. Плюс — не требует дополнительного пространства; минус — фрагментация (много мелких свободных зон).

Mark-and-Compact: после mark пройти и переместить живые объекты (compact) в одно место, чтобы убрать фрагментацию. Compact сложнее и обычно стоит дорого (перемещение + фиксация ссылок).

Mark-and-Copy (evacuation): вариант compact, но делается копированием в другую область; часто применяют для молодых поколений (копирование дешевле, т.к. живых немного).


#### Parallel vs Concurrent (и safepoints)

Parallel (STW multi-threaded): GC-работа выполняется многопоточно, но приложение останавливается (stop-the-world) на время GC-фазы. Подходит для throughput-ориентированных задач. Примеры: Parallel GC, Parallel Old.

Concurrent: часть работы выполняется параллельно с приложением (минимизируя паузы). Но возникают сложности с согласованностью (поэтому нужны барьеры — write/read barriers) и дополнительные накладные расходы. Примеры: G1 (частично concurrent), Shenandoah, ZGC (много concurrent фазы).

Safepoints: некоторые GC-фазы требуют, чтобы все Java-потоки были в точках, где их можно безопасно остановить — это и есть safepoints. Частые safepoints приводят к паузам; некоторые collector'ы уменьшают число STW-фаз за счёт concurrent-фаз.
